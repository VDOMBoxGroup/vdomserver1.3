...
class VDOM_sshfs_drive(VDOM_storage_driver):
	...
	dev = property(lambda self: self.__dev)
	login=property(lambda self: self.__ssh_login)
	password=property(lambda self: self.__ssh_password)
	host=property(lambda self: self.__ssh_hostname)
	port=property(lambda self: self.__ssh_port)
	remote_path=property(lambda self: self.__ssh_remote_path)
