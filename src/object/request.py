...
class VDOM_request(object):
	...
	def __get_vdom(self):
		return src.managers.request_manager.current.vdom
	...
	def __get_arguments(self):
		return src.managers.request_manager.current.arguments().arguments()
	...
	def __get_files(self):
		return src.managers.request_manager.current.files
	...
	def __get_application_id(self):
		return src.managers.request_manager.current.app_id()
	...
	def __get_container_id(self):
		return src.managers.request_manager.current.container_id
	...
	def redirect(self, to):
		...
		src.managers.request_manager.current.redirect(to)
		src.engine.engine.terminate()

	def application(self):
		return src.managers.request_manager.current.application()
	
	def __get_database_manager(self):
		return src.managers.database_manager
	...
	def __get_email_manager(self):
		return src.managers.email_manager
	...
	def __get_resource_manager(self):
		return src.managers.resource_manager
	...
	def __get_user_manager(self):
		return src.managers.user_manager
	...
	def __get_acl_manager(self):
		return src.managers.acl_manager
	...
	def __get_app_vhname(self):
		return src.managers.request_manager.current.app_vhname
	...
	def write(self, some, continue_render=False):
		src.managers.request_manager.current.write(some)
		if not continue_render:
			src.engine.engine.terminate()

	def write_handler(self, some):
		src.managers.request_manager.current.write_handler(some)

	def add_header(self, name, value):
		src.managers.request_manager.current.add_header(name, value)

	def add_client_action(self, id, data):
		src.managers.request_manager.current.add_client_action(id, data)

	def binary(self, b = None):
		return src.managers.request_manager.current.binary(b)

	def set_nocache(self):	
		return src.managers.request_manager.current.set_nocache()
	
	def send_file(self,filename, length, handler, content_type=None):
		return src.managers.request_manager.current.send_file(filename, length, handler, content_type)
	
	def __get_cookies(self):
		return src.managers.request_manager.current.cookies()
	...
	def __get_environment(self):
		return src.managers.request_manager.current.environment().environment()
