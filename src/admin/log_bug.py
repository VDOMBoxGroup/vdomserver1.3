...
def run(request):
	...
	if not sess.value("appmngmtok"):
		...
		file = VDOM_CONFIG["STORAGE-DIRECTORY"] + "/log/bug"
		if current > 0:
			file += "." + str(current)
		data = ""
		...
		nav = ""
		if 0 == current:
			nav = """<a href="/log-bug.py?file=1">Next&gt;</a>"""
		elif VDOM_CONFIG["LOG-FILE-COUNT"] == current:
			nav = """<a href="/log-bug.py?file=%d">&lt;Back</a>""" % (current - 1)
		else:
			nav = """<a href="/log-bug.py?file=%d">&lt;Back</a>...<a href="/log-bug.py?file=%d">Next&gt;</a>""" % (current - 1, current + 1)
...
<body>
<center>
<p align="left"><span class="Texte"><a href="log.py">Log</a> &gt; Bug</span></p>
