...
<form name="form1" method="post" action="/users-modif-rights.py?uid=%s&appid=%s&oid=%s">
...
<br>
			...
			content = """<span class="Texte-page">User: %s<br></span>""" % user.login
			content += """<p class="Texte-page" align="center">Click on object and Modify access</p>"""

			content += """<script type="text/javascript">
				var Tree2 = new Array;"""
			content += """ Tree2[0]  = "1|0|%s (Application, id: %s)|users-modif-rights-form.py?uid=%s&appid=%s&oid=%s";""" % (app.name.encode("utf-8"), appid, _uid, _appid, _appid)
			objlist = []
			TabParent = []
			cpt = 1
			...
			while len(objlist) > 0:
				...
				content += """ Tree2[%s]  = "%s|%s|%s (type: %s, id: %s)|users-modif-rights-form.py?uid=%s&appid=%s&oid=%s";""" % (cpt, cpt+1, IdParent, obj.name.encode("utf-8"), obj.type.name.encode("utf-8"), obj.id, _uid, _appid, "_".join(obj.id.split("-")))
				i = 0
				cpt += 1
...
<script type="text/javascript">
	createTree(Tree2,1);
			...
			content = """<p align="center" class="Texte-page">Select application</p>
<form name="form1" method="post" action="/users-modif-rights.py?uid=%s">
<table align="center" cellpadding="0" cellspacing="0">
  <TR>
...
<body topmargin="2">
<p class="Texte"><a href="users.py">Users</a> &gt; <a href="users-modif.py">Select user</a> &gt; User rights</p>
