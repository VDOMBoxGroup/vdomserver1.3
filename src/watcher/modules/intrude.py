...
def intrude(options):
	if "raise" in options:
		try:
			value=options["raise"]
			if not pattern.match(value): raise ValueError
			exception=eval(value)
			if not issubclass(exception, Exception): raise ValueError 
		...
		try:
			thread=search_thread(options["thread"])
			if thread is None: raise OptionError("Unable to find thread")
			if not isinstance(thread, VDOM_thread): raise OptionError("Thread is not smart")
		except OptionError as error:
			yield "<reply><error>%s</error></reply>"%error
		else:
			thread.stop()
