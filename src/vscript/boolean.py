...
class boolean(object):
	...
	def __init__(self, value):
		self.value=v_true_value if value else v_false_value
	...
	def eq_empty(self, another):
		return boolean(v_true_value) if another.value==0 else boolean(v_false_value)
	...
	def and_null(self, another):
		return v_null if self.value else boolean(v_false_value)

	def or_null(self, another):
		return boolean(v_true_value) if self.value else v_null
	...
	def eq_double(self, another):
		return boolean(v_true_value) if another.value==another.double else boolean(v_false_value)
	...
	def and_double(self, another):
		return integer(self.value&int(round(another.double)))
	...
	def eq_string(self, another):
		keyword=u"True" if self.value==v_true_value else u"False"
		return boolean(v_true_value) if keyword==another.value else boolean(v_false_value)
	...
	def and_boolean(self, another):
		return boolean(self.value&another.value)
	...
	def __add__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.add_table.get(type(another), boolean.add_unknown)(self, another)
	...
	def __sub__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.sub_table.get(type(another), boolean.sub_unknown)(self, another)
		except OverflowError, error:
			exclass, exexception, extraceback=sys.exc_info()
			raise errors.overflow, None, extraceback
	...
	def __mul__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.mul_table.get(type(another), boolean.mul_unknown)(self, another)
		except OverflowError, error:
			exclass, exexception, extraceback=sys.exc_info()
			raise errors.overflow, None, extraceback
	...
	def __div__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.div_table.get(type(another), boolean.div_unknown)(self, another)
		except OverflowError, error:
			exclass, exexception, extraceback=sys.exc_info()
			raise errors.overflow, None, extraceback
	...
	def __floordiv__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.floordiv_table.get(type(another), boolean.floordiv_unknown)(self, another)
		except OverflowError, error:
			exclass, exexception, extraceback=sys.exc_info()
			raise errors.overflow, None, extraceback
		except ZeroDivisionError, error:
	...
	def __mod__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.mod_table.get(type(another), boolean.mod_unknown)(self, another)
		except OverflowError, error:
			exclass, exexception, extraceback=sys.exc_info()
			raise errors.overflow, None, extraceback
		except ZeroDivisionError, error:
	...
	def __pow__(self, another):
		if isinstance(another, (types.function, types.method, generic)):
			another=another()
		try:
			return boolean.pow_table.get(type(another), boolean.pow_unknown)(self, another)
		except OverflowError, error:
			exclass, exexception, extraceback=sys.exc_info()
			raise errors.overflow, None, extraceback
	...
	def __eq__(self, another):
		return boolean.eq_table.get(type(another), boolean.eq_unknown)(self, another)

	def __ne__(self, another):
		return boolean.ne_table.get(type(another), boolean.ne_unknown)(self, another)

	def __lt__(self, another):
		return boolean.lt_table.get(type(another), boolean.lt_unknown)(self, another)

	def __gt__(self, another):
		return boolean.gt_table.get(type(another), boolean.gt_unknown)(self, another)

	def __le__(self, another):
		return boolean.le_table.get(type(another), boolean.le_unknown)(self, another)

	def __ge__(self, another):
		return boolean.ge_table.get(type(another), boolean.ge_unknown)(self, another)
	...
	def __and__(self, another):
		return boolean.and_table.get(type(another), boolean.and_unknown)(self, another)

	def __or__(self, another):
		return boolean.or_table.get(type(another), boolean.or_unknown)(self, another)

	def __xor__(self, another):
		return boolean.xor_table.get(type(another), boolean.xor_unknown)(self, another)
	...
	def __repr__(self):
		return "BOOLEAN@%s:%s"%(object.__repr__(self)[-9:-1], repr(self.value))
